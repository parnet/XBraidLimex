...
//	perform checks
        if (prevSol->size() < m_prevSteps) UG_THROW("LinearImplicitEuler::prepare_step:"
...
//	update scalings
        ...
        std::cout << "PREP: " << m_vScaleMass[0] << ", " << m_vScaleStiff[0] << ", " << m_dt << ", "
                  << m_pPrevSol->time(0) << std::endl;
...
//	prepare time step (elemDisc-wise)
        try {
            ...
            if (m_spGammaDisc.valid()) { m_spGammaDisc->prepare_timestep(m_pPrevSol, m_futureTime); }
        }
...
//	perform checks
        if (prevSol->size() < m_prevSteps) UG_THROW("LinearImplicitEuler::prepare_step_elem:"
                                                    " Number of previous solutions must be at least " <<
                                                                                                      m_prevSteps
                                                                                                      << ", but only "
                                                                                                      << prevSol->size()
...
// 	prepare timestep
    ...
    adjust_solution(vector_type &u, const GridLevel &gl) {
        UG_DLOG(LIB_LIMEX, 5, "LinearlyImplicitEuler:adjust_solution" << &u << std::endl);PROFILE_BEGIN_GROUP(
                LinearImplicitEuler_adjust_solution, "discretization LinearImplicitEuler");
...
//	perform checks whether 'currSol' is a solutionTimeSeries only with the new values
        if (currSol->time(0) != m_futureTime) UG_THROW("LinearImplicitEuler::finish_step_elem:"
 ...
 * */
    ...
    assemble_jacobian(matrix_type &J_limex, const vector_type &u, const GridLevel &gl) {
        ...
        //	perform checks
        if (m_pPrevSol->size() < m_prevSteps) UG_THROW("LinearImplicitEuler::assemble_jacobian:"
                                                       " Number of previous solutions must be at least " <<
                                                                                                         m_prevSteps
                                                                                                         << ", but only "
                                                                                                         << m_pPrevSol->size()
        ...
        try {
            ...
            if (!m_useCachedMatrices) {
                ...
                UG_DLOG(LIB_LIMEX, 3, "> Computed Mk (" << &J_limex << " : " << J_limex <<
                                                        " at " << m_pPrevSol->oldest_time() << ", " << GetNNZs(J_limex)
                                                        << " nonzeros)" << std::endl);
                ...
                if (m_bMatrixJNeedsUpdate) {
                    ...
                    if (m_spGammaDisc.valid()) {
                        ...
                        UG_DLOG(LIB_LIMEX, 3, " at " << m_pPrevSol->oldest_time() << ", " << GetNNZs(myGamma) << " nnz)"
                                                     << std::endl);
 ...
 * Must return : d_A(k-1):= tau * F(k-1) - A(k-1) u(k-1) */
    ...
    assemble_defect(vector_type &d, const vector_type &u, const GridLevel &gl) {
        ...
        //	perform checks
        if (m_pPrevSol->size() < m_prevSteps) UG_THROW("LinearImplicitEuler::assemble_defect:"
                                                       " Number of previous solutions must be at least " <<
                                                                                                         m_prevSteps
                                                                                                         << ", but only "
                                                                                                         << m_pPrevSol->size()
...
/* RHS (non-linear system) */
    ...
    assemble_rhs(vector_type &b, const vector_type &u, const GridLevel &gl) {
        UG_ASSERT(0, "Really wanna use me???");PROFILE_BEGIN_GROUP(LinearImplicitEuler_assemble_rhs,
                                                                   "discretization LinearImplicitEuler");
//	perform checks
        if (m_pPrevSol->size() < m_prevSteps) UG_THROW("LinearImplicitEuler::assemble_linear:"
                                                       " Number of previous solutions must be at least " <<
                                                                                                         m_prevSteps
                                                                                                         << ", but only "
                                                                                                         << m_pPrevSol->size()
...
/* RHS (linear system) */
    ...
    assemble_rhs(vector_type &b, const GridLevel &gl) {
        UG_ASSERT(0, "Really wanna use me???");PROFILE_BEGIN_GROUP(LinearImplicitEuler_assemble_rhs,
                                                                   "discretization LinearImplicitEuler");
//	perform checks
        if (m_pPrevSol->size() < m_prevSteps) UG_THROW("LinearImplicitEuler::assemble_rhs:"
                                                       " Number of previous solutions must be at least " <<
                                                                                                         m_prevSteps
                                                                                                         << ", but only "
                                                                                                         << m_pPrevSol->size()
